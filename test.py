...
class TestSQLMongo(unittest.TestCase):
    ...
    def test_01_spec_str(self):
        ...
        """
        input_dict = {'a': 1, "b": [{'c': 'cc'}, {
            'd': 0}], "e": {'f': 'g', 'h': ['i', 'j']}}
        comp_str = "{a:1, b:[{c:'cc'}, {d:0}], e:{f:'g', h:['i', 'j']}}"
        output_str = spec_str(input_dict)
        assert comp_str.translate(str.maketrans('', '', '1234567890')) == output_str.translate(str.maketrans('', '', '1234567890')), \
    ...
    def test_03_parsing(self):
        ...
        for test_case in self.test_data:
            ...
            assert test_case['mongo'].replace(" ", "") == mongo_cmd.replace(" ", ""),\
                    "Expected {} got {} when sql is {}".format(test_case['mongo'],mongo_cmd,test_case['sql'])
